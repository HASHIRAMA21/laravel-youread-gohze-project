    ...
    >
      ...
      <div class="parent fixed">
        ...
        <div class="border-b border-gray-400">
          <div class="w-2/3 sm:w-1/2 border-b border-vertbleu text-vertbleu px-2 py-2">Remplir le formulaire</div>
        </div>
          <div class="mt-4">
            <div class="flex text-neutral-500 container-infos justify-between">
              <div class="flex flex-col">
                <div class="py-4" v-if="!update"><p>ID</p></div>
                <div class="py-4"><p>Libellé</p></div>
                <div class="py-4"><p>Droit</p></div>
              </div>
              <div class="flex flex-col">
                <div class="py-4" v-if="!update">
                  <input
                    v-model="item.id"
                    class="
                ...
                <div class="py-4">
                  ...
                  >
                    ...
                    <option>Créer un chapitre</option>
                    <option>Supprimer un chapitre</option>
                    <option>Publier un chapitre</option>
                    <option>Ajouter des membres</option>
                    <option>Retirer des membres</option>
                  </select>
                </div>
              </div>
            </div>
...
export default {
  ...
  props: {
    showModal: false,
    closeFunction: false,
    update: { type: Boolean, default: false }, //true si c'est pour update, false si c'est pour créer.
    action: { type: String, default: "create" },
    entite: { type: String, required: true }, //La table sur laquelle on veut agir (role, chapitre, membre, etc.)
    item: { type: Object, required: true }, //L'occurrence qu'on veut créer
  },

  setup(props){
    const titre = computed(() => {
      return props.update ? "Editer un rôle" : "Ajouter un rôle";
    });
