<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use App\Models\Comment;
use App\Models\Publication;
use App\Models\User;
use App\Notifications\SomeoneReplied;
use Exception;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;

class CommentController extends Controller
{
    /**
     * Display a listing of the resource.
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function index(Request $request): JsonResponse
    {
        $publication_id = $request->publication_id;

        $comments = Comment::with('user:id,username,avatar',
            'commentLikes:id,comment_id,user_id,status',
            'commentDislikes:id,comment_id,user_id,status')
            ->select(['id', 'user_id', 'publication_id', 'comment_text', 'parent_id', 'created_at'])
            ->withCount('replies', 'commentLikes', 'commentDislikes')
            ->where('publication_id', $publication_id)
            ->whereNull('parent_id')
            ->latest()
            ->paginate(20);

        return response()->json($comments);
    }

    /**
     * Store a newly created resource in storage.
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function store(Request $request): JsonResponse
    {
        try {
            $user_id = auth()->user()->id;
            $publication_id = $request->publication_id;
            $parent_id = $request->parent_id;
            $comment_text = $request->comment_text;
            $replied_to_id = $request->replied_to_id;

            $this->validate($request, [
                'publication_id' => 'required',
                'comment_text' => 'required|max:255',
                'parent_id' => 'sometimes',
                'replied_to_id' => 'sometimes'
            ]);

            $comment = Comment::create([
                'user_id' => $user_id,
                'publication_id' => $publication_id,
                'comment_text' => $comment_text,
                'parent_id' => $parent_id,
                'replied_to_id' => $replied_to_id
            ]);

            if (!empty($parent_id && $replied_to_id)) {
                $user = Comment::findOrFail($replied_to_id)->user_id;
                $toUser = User::findOrFail($user);
                $fromUser = User::findOrFail($user_id);
                $publication = Publication::findOrFail($publication_id);
                $toUser->notify(new SomeoneReplied($fromUser, $comment, $publication));
                $toUser->commentPushNotification($fromUser, $comment, $publication);
            }
            return response()->json($comment, 201);
        } catch (Exception $exception) {
            return response()->json($exception->getMessage(), 500);
        }
    }

    /**
     * Display the specified resource.
     *
     * @param int $id
     * @return JsonResponse
     */
    public function show($id): JsonResponse
    {
        $replies = Comment::with('user:id,username,avatar',
            'commentLikes:id,comment_id,user_id,status',
            'commentDislikes:id,comment_id,user_id,status')
            ->where('parent_id', '=', $id)
            ->select('id', 'user_id', 'publication_id', 'comment_text', 'parent_id', 'created_at')
            ->latest()
            ->paginate(20);

        return response()->json($replies);
    }

    /**
     * Update the specified resource in storage.
     *
     * @param Request $request
     * @param int $id
     * @return \Illuminate\Http\Response
     */
    public function update(Request $request, $id)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     *
     * @param int $id
     * @return JsonResponse
     */
    public function destroy($id): JsonResponse
    {
        try {
            if (auth()->user()->id == Comment::findOrFail($id)->user_id) {
                $review = Comment::findOrFail($id);
                $review->delete();
                return response()->json(['message' => 'Comment deleted.'], 200);
            }
            return response()->json([
                'message' => 'You do not have permission to delete this comment.'
            ], 403);
        } catch (Exception $exception) {
            return response()->json($exception->getMessage(), 500);
        }
    }
}